...
if __name__ == '__main__':
    ...
    atom_nums = [len(mol) for mol in atom_features]
    max_num_atoms = max(atom_nums)  # calculate maximum number of atom number

    adjacency_lists = [feature.get_adjacency_list() for feature in molecule_features]
    adjacency_matrices = [adj_list_to_adj_matrix(adj_list, max_num_atoms) for adj_list in adjacency_lists]
    pad_nums = [max_num_atoms - n for n in atom_nums]
    atom_features_padded = list()
    for (adj, pn) in zip(atom_features, pad_nums):
        atom_features_padded.append(np.pad(adj, pad_width=((0, pn), (0, 0)), mode='constant', constant_values=0))
    ...
    ecfp_features = [dc.feat.rdkit_grid_featurizer.compute_ecfp_features(
        mol, ecfp_degree=ecfp_degree, ecfp_power=ecfp_power) for mol in molecules]
